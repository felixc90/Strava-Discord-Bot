...
module.exports = {
	data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('day')
                .setDescription('Data by day!')
                .addUserOption(option => option.setName('user').setDescription('Comparing with user')))
        .addSubcommand(subcommand =>
            subcommand
                .setName('week')
                .setDescription('Data by week!')
                .addUserOption(option => option.setName('user').setDescription('Comparing with user'))),
        async execute(interaction) {
            const unit_of_time = interaction.options._subcommand
            ...
            for (hoisted_options of interaction.options._hoistedOptions) {
                run_data = await getRunData(hoisted_options.user.id, unit_of_time, -1)
                datasets.push({
                    username : hoisted_options.user.username,
                    dates : run_data.dates,
                    data : run_data.distances
                })
            }
