...
async function getGraph(interaction) {
    ...
    let user = await User.findOne({discord_id : interaction.user.id}, 'statistics days_last_active')
    ...
    if (interaction.options._hoistedOptions.length != 0) {
        user1 = await User.findOne({discord_id : interaction.options._hoistedOptions[0].user.id}, 'statistics days_last_active')
